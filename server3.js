...
wss.on('connection', ws => {
    ...
    ws.on('message', async message => {
        ...
        if (parsedMessage.type === 'joinCall') {
            ...
            try {
                ...
                try {
                    ...
                    const trackName = trackData.tracks[0].trackName; // Extract trackName
                    ws.send(JSON.stringify({
                        type: 'trackAdded',
                        response: addTrackResponse,
                        sessionId: sessionId,
                        trackName: trackName // Send the trackName
                    }));
                    ...
                    for (const remoteClientId in rooms[roomId]) {
                        if (remoteClientId !== clientId) {
                            ws.send(JSON.stringify({
                                type: 'newTrackAvailable',
                                clientId: clientId,
                                sessionId: sessionId,
                                trackName: trackName  // Send the new client's trackName
                            }));
        ...
        } else if (parsedMessage.type === 'addRemoteTrack') {
            const remoteSessionId = parsedMessage.sessionId;
            const remoteTrackData = parsedMessage.trackData;
            const clientId = parsedMessage.clientId;
            console.log(`[Server] ➕ Adding remote track ${remoteTrackData} to ${remoteSessionId}`);
            try {
                const addTrackResponse = await addTrackToCloudflareSession(remoteSessionId, remoteTrackData);
                ...
                console.error(`[Server] Error adding remote track for ${remoteSessionId}:`, trackError);
                ws.send(JSON.stringify({
                    type: 'error',
                    message: trackError.message
                }));
